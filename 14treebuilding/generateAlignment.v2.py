...
def align(element):
	...
	commands = """
	python pick_loci_and_fix_double.v2.py {threshold} conus.excludethese.{threshold}
	python concatenatedFile.py conus_phylogeny_{threshold}.nexus
	python trimmfile.py conus_phylogeny_{threshold}.nexus conus_phylogeny_headed_{threshold}.nexus
	python nexus2phylip.py conus_phylogeny_headed_{threshold}.nexus conus_phylogeny_{threshold}.phylip
	python countcompleteness.py conus_phylogeny_headed_{threshold}.nexus conus_phylogeny_headed_{threshold}.completeness conus.excludethese.v2.{threshold}
	""".format(**variables)
