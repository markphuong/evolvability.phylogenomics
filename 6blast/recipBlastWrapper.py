...
def align(element):
	...
	commands = """
	cp /pylon5/bi4s86p/phuong/evolvability/5mapping/{sample}.vcf $LOCAL
	cp /pylon5/bi4s86p/phuong/evolvability/5mapping/{sample}_*.NI $LOCAL
	makeblastdb -dbtype nucl -in {sample}_assemblies_clustered.fasta.NI
	blastn -query {sample}_assemblies_clustered.fasta.NI -db conus.baits.2017.final -outfmt 6 -word_size 11 -out {sample}.blastoutput -evalue 1e-10
	blastn -query conus.baits.2017.final -db {sample}_assemblies_clustered.fasta.NI -outfmt 6 -word_size 11 -out {sample}.recipblastoutput -evalue 1e-10
	...
	cp {sample}.blastoutput /pylon5/bi4s86p/phuong/evolvability/6blast
	cp {sample}.recipblastoutput /pylon5/bi4s86p/phuong/evolvability/6blast
	cp {sample}.cov /pylon5/bi4s86p/phuong/evolvability/6blast
	cp {sample}_removed /pylon5/bi4s86p/phuong/evolvability/6blast
	cp {sample}_filtered_recipblast /pylon5/bi4s86p/phuong/evolvability/6blast
	""".format(**variables)
