...
def align(element):
	...
	commands = """
	cp /pylon2/bi4s86p/phuong/evolvability/9blastagain/{sample}_phylo_contigs.fasta ./
	cp /pylon2/bi4s86p/phuong/evolvability/10mapagain/{sample}.vcf ./
	python get_cov_het.py {sample}_phylo_contigs.fasta {sample}.vcf {sample}.6.1.cov {sample}.6.1.het
	cp {sample}.6.1.cov /pylon2/bi4s86p/phuong/evolvability/11cov.het
	cp {sample}.6.1.het /pylon2/bi4s86p/phuong/evolvability/11cov.het
	""".format(**variables)
